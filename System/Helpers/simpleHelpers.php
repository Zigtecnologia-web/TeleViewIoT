...
function uploadBase64Image($filesName)
{
    try {
        if (isset($_FILES[$filesName]['tmp_name']) && is_uploaded_file($_FILES[$filesName]['tmp_name'])) {
        ...
        // Log the error message and continue with the script, or rethrow the exception
...
function real($valor)
{
    return number_format(($valor != 0) ? $valor : 0, 2, ',', '.');
}
...
function imageDirectory($folderName)
{
    ...
    if (filter_var($_ENV['SHARED_HOST'], FILTER_VALIDATE_BOOLEAN)) {
        return $path;
    }
    return "public/$path";
}
